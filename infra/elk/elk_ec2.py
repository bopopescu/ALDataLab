...
# Default instance type for both main and subordinate
ELK_EC2_VERSION="1.0.0"
DEFAULT_INSTANCE_TYPE="m3.xlarge"
DEFAULT_REGION="us-east-1"
DEFAULT_AVAILIBILITY_ZONE="us-east-1a"
DEFAULT_INSTANCE_PROFILE="elasticsearch"
...
def parse_args():
    parser = OptionParser(
        prog="elk-ec2",
        version="%prog {v}".format(v=ELK_EC2_VERSION),
    ...
    parser.add_option(
        "-m", "--main-instance-type", default=DEFAULT_INSTANCE_TYPE,
    ...
    parser.add_option(
        "-z", "--zone", default=DEFAULT_AVAILIBILITY_ZONE,
    ...
    parser.add_option(
        ...
        help="If you have multiple profiles (AWS or boto config), you can configure " +
             ...
             "to the main instead of profile option  (default: %default)")
    parser.add_option(
    ...
    parser.add_option(
        "--main-user-data", type="string", default="user_data/kibana.sh",
        help="Path to a user-data file for main (most AMIs interpret this as an initialization script)")
    parser.add_option(
        "--subordinate-user-data", type="string", default="user_data/elastic.sh",
        help="Path to a user-data file for subordinates (most AMIs interpret this as an initialization script)")
    parser.add_option(
    ...
    parser.add_option(
        "--instance-profile-name", default=DEFAULT_INSTANCE_PROFILE,
...
def get_ami(opts):
    ...
    if opts.region in DEFAULT_AMIS:
        ami = DEFAULT_AMIS[opts.region][instance_type]

    if ami is None:
        print("ERROR: Can't find suitable AMI for region %s and instance type %s" % (opts.region,instance_type))
        sys.exit(1)

    print("Selected AMI: " + ami)
    return ami
...
def launch_cluster(conn, opts, cluster_name):
    template_vars = {
        ...
        'main_security_group': cluster_name + "-main",
        'subordinate_security_group': cluster_name + "-subordinates",
    ...
    main_group = get_or_make_group(conn, template_vars['main_security_group'], opts.vpc_id)
    subordinate_group = get_or_make_group(conn, template_vars['subordinate_security_group'], opts.vpc_id)
    ...
    if main_group.rules == []:  # Group was just now created
        if opts.vpc_id is None:
            ...
            main_group.authorize(src_group=discovery_group)
        else:
    ...
    if subordinate_group.rules == []:  # Group was just now created
        if opts.vpc_id is None:
            ...
            subordinate_group.authorize(src_group=discovery_group)
        else:
    ...
    if discovery_group.rules == []:  # Group was just now created
        if opts.vpc_id is None:
            discovery_group.authorize(src_group=main_group)
            discovery_group.authorize(src_group=subordinate_group)
    ...
    main_user_data_content = get_user_data(opts.main_user_data,template_vars)
    subordinate_user_data_content = get_user_data(opts.subordinate_user_data,template_vars)
    ...
    if opts.spot_price is not None:
        ...
        for zone in zones:
            ...
            subordinate_reqs = conn.request_spot_instances(
                ...
                security_group_ids=[subordinate_group.id,discovery_group.id] + additional_group_ids,
                instance_type=opts.instance_type,
                block_device_map=block_map,
                subnet_id=opts.subnet_id,
                placement_group=opts.placement_group,
                user_data=subordinate_user_data_content,
    ...
    if existing_mains:
        ...
        if opts.spot_price is not None:
            ...
            print("Requesting main as spot instance with price $%.3f" % (opts.spot_price))
            main_reqs = conn.request_spot_instances(
                    price=opts.spot_price,
                    image_id=opts.ami,
                    key_name=opts.key_pair,
                    launch_group="main-group-%s" % cluster_name,
                    security_group_ids=[main_group.id,discovery_group.id] + additional_group_ids,
                    ...
                    subnet_id=opts.subnet_id,
                    placement_group=opts.placement_group,
                    user_data=main_user_data_content,
                    instance_profile_name=opts.instance_profile_name)
            main_req_id = main_reqs[0].id
            ...
            try:
                while True:
                    ...
                    for r in reqs:
                        id_to_req[r.id] = r
                    main_instance_ids = []
                    if main_req_id in id_to_req and id_to_req[main_req_id].state == "active":
                        main_instance_ids.append(id_to_req[main_req_id].instance_id)
                        print("Main granted")
                        reservations = conn.get_all_reservations(main_instance_ids)
                        main_nodes = []
                        for r in reservations:
                            main_nodes += r.instances
                        break
                    else:
                        print("Main not granted yet, waiting longer")
            except:
                print("Canceling spot instance request for main")
                conn.cancel_spot_instance_requests([main_req_id])
                sys.exit(0)
        else:
            main_res = image.run(
                key_name=opts.key_pair,
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key,cluster_name):
...
def get_user_data(filename, template_vars={}):
    ...
    if filename:
        with open(filename,'r') as file:
            ...
            for key in template_vars:
                user_data_content = user_data_content.replace("{{" + key + "}}", template_vars[key])

    return user_data_content.encode('utf-8')
...
def real_main():
    ...
    if action == "launch":
        ...
        setup_cluster(conn, main_nodes, subordinate_nodes, opts, True, cluster_name)
        ...
        setup_cluster(conn, main_nodes, subordinate_nodes, opts, False, cluster_name)
